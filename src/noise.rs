// Esto es completamente estandar de Rust
use rand::Rng;
// A los RNG de Rust que implementan .sample se les puede pasar una distribución.
use rand_distr::{Normal};
use crate::fftconvolve::fftconvolve_envuelto;
...
fn impulse_to_noise<R: Rng>(impulso: SigmaH, mut rng: R) -> Vec<f32> {
    // pattern Matching Estilo Haskell, porque me cobran caro los enter.
...
//
// La función que ahora llamo impulse_to_noise es un pedacito de adentro de la función create_noise de
// python.
// Create noise recibe una lista de modelos y devuelve la suma. En este caso podría ser
// tranquilamente una suma de llamadas a impulse_to_noise.
//
...
mod tests {
    ...
    use crate::noise::impulse_to_noise;
    ...
    #[test]
    fn test_white_identity_ok(){
        // Este test funciona aún si la función fftconvolve_envuelta es la 
        // identidad. Porque la ventana que resulta es [1 0 0 0 0]
        // Sirve para testear justamente un caso donde fftconvolve_envuelta es
        // salteada
        let m: usize = 20;
        let sigma: f32 = 1.0; //5.4881350392732475;
        ...
        let response = impulse_to_noise( impulso, rng);
